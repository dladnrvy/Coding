					...
					<div id="contents">
						<div id="con">
							...
							<form name="form" method="post" onSubmit="return false;">
								...
								<button onClick="GetList();" class="btn btn-warning" id="bu">영상검색</button>
								<br>
								<br>
							</form>
							...
							<br><br>
								<form id="photoform" enctype="multipart/form-data">
									...
									<input type="submit" value="사진수정" class="btn btn-warning">
								</form>
							</div>
	...
	<script>
					...
					.ajax({
						...
						success : function(data) {
							...
							for (i = 0; i < data.length; i++) {
								...
								html += '<div id="idx" class="idx">' + data[i].idx
										+ '</div>\n';
								html += '<div class="list"><img onclick="textList('
										+ data[i].idx
										+ ')" id="pho" src="http://localhost:8080/info/uploadfile/'
										+ data[i].photo + '"/></div>\n';
								html += '<br><div class="list">제목 : '
										+ data[i].title + '</div>\n';
								html += '<div class="list">작성자 :  '
										+ data[i].uId + '</div>\n';
								html += '<div class="list">'
										+ data[i].date.substr(0, 16)
										+ '</div>\n';
								html += '</div>'
								/*html += '수정'*/
							}
		...
		function textList(idx) {
			...
			$.ajax({
						...
						success : function(data) {
							...
							html += '<div id="choice">';
							html += '<a href="main.jsp" class="btn btn-warning" id="lists">리스트</a><br><br>';
							if ('${sessionScope.id}' == data.uId) {
								html += '<button onclick ="del('+data.idx+')" class="btn btn-warning" id="del">삭제</button>';
								html += '<button onclick ="put('+ data.idx+')" class="btn btn-warning" id="up">수정</button><br><br>';
		...
		function GetList(GetToken) {
			...
			var youtubeUrl = "https://www.googleapis.com/youtube/v3/search?part=snippet&order=relevance"
					...
					+ "&type=video&videoDefinition=high&maxResults=3&key=AIzaSyAcezwf-iUpTYkBw2neqfl99jL8xFhSRfU";
