...
<%		
		...
		BookSelectService service2 = BookSelectService.getInstance();
		
		JoinModel joinmodel = service2.selectBook(book_name);
		
		
		if(joinmodel.getBook_ren().trim().equals("N")){
			
			%>
			<script>alert('이미 대여중인 책 입니다.');
		...
		<script>alert('로그인을 해주세요.');
			location.href = "/lib/users/loginLogout/loginForm.jsp";
		</script>
